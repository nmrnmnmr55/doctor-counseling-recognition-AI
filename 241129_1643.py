...
def main():
    ...
    if LAST_BUTTON_PRESS_TIME:
        ...
        remaining_minutes = max(0, AUTO_SHUTDOWN_MINUTES - time_since_last_button.total_seconds() / 60)
        st.info(f"最後の操作からの残り時間: {int(remaining_minutes)}分")
    else:
        time_since_start = current_time - APP_START_TIME
        remaining_minutes = max(0, AUTO_SHUTDOWN_MINUTES - time_since_start.total_seconds() / 60)
        st.info(f"アプリ起動からの残り時間: {int(remaining_minutes)}分")
